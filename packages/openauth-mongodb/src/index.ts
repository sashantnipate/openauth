import { OpenAuthAdapter, SessionRepository, Session } from "@openauth/sdk";
...
class MongoSessionRepository implements SessionRepository {
  private mapDocument(doc: any): Session {
    return {
      id: doc._id.toString(),
  ...
  async findByToken(token: string): Promise<Session | null> {
    const doc = await SessionModel.findOne({ token, active: true }).lean();
    return doc ? this.mapDocument(doc) : null;
  }

  async findByUserId(userId: string): Promise<Session[]> {
    if (!Types.ObjectId.isValid(userId)) return [];
    const docs = await SessionModel.find({ userId: new Types.ObjectId(userId), active: true }).lean();
  ...
  async delete(id: string): Promise<void> {
    // Soft delete session state by marking deactivated, or wipe hard block entry
...
export class MongooseOpenAuthAdapter implements OpenAuthAdapter {
  ...
  public sessions = new MongoSessionRepository(); // CONNECTED REAL REPOSITORY
}